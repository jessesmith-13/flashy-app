...
export const useStore = create<AppState>((set, get) => ({
  ...
  addFriend: (userId) =>
    set((state) => ({ friends: [...state.friends, userId] })),
  removeFriend: (userId) =>
    set((state) => ({ friends: state.friends.filter((id) => id !== userId) })),
  addFriendRequest: (userId) =>
    set((state) => ({
      friendRequests: state.friendRequests.some((req) => req.id === userId)
        ? state.friendRequests
        : [...state.friendRequests, { id: userId, name: "", email: "" }],
    })),
  ...
  addPendingFriendRequest: (userId) =>
    set((state) => ({
      pendingFriendRequests: state.pendingFriendRequests.includes(userId)
        ? state.pendingFriendRequests
        : [...state.pendingFriendRequests, userId],
    })),
  ...
  setMentionNotifications: (notifications) =>
    set({ mentionNotifications: notifications }),
    ...
    if (state.accessToken) {
      try {
        ...
        const response = await fetch(
          ...
          }/functions/v1/server/study/sessions`,
          {
            method: "POST",
            headers: {
  ...
  darkMode:
    ...
    localStorage.getItem("flashy-darkMode") === "true",
  ...
  setViewingCommunityDeckId: (deckId) =>
    set({ viewingCommunityDeckId: deckId }),
