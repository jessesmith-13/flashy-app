...
export function BulkAddCardsDialog({
  ...
  const [step, setStep] = useState<"select-count" | "fill-cards">(
    "select-count"
  );
  ...
  const [uploadingAudioCardId, setUploadingAudioCardId] = useState<
    string | null
  >(null);
  const [uploadingAudioField, setUploadingAudioField] = useState<
    "front" | "back" | null
  >(null);
  ...
  const handleRemoveIncorrectAnswer = (cardId: string, index: number) => {
    ...
    const newIncorrectAnswers = card.incorrectAnswers.filter(
      (_, i) => i !== index
    );
    handleCardChange(cardId, { incorrectAnswers: newIncorrectAnswers });
  };
  ...
  const handleRemoveAcceptedAnswer = (cardId: string, index: number) => {
    ...
    const newAcceptedAnswers = card.acceptedAnswers.filter(
      (_, i) => i !== index
    );
    handleCardChange(cardId, { acceptedAnswers: newAcceptedAnswers });
  };
        ...
        {step === "select-count" ? (
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Fill in the details for each card. All card types and features
                are supported.
              </DialogDescription>
            </DialogHeader>
                ...
                {cards.map((card, index) => (
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      {/* Card Type Selector */}
                      <div>
                        <Label htmlFor={`${card.id}-type`} className="text-sm">
                          Card Type
                        </Label>
                        <Select
                          value={card.cardType}
                          onValueChange={(value) =>
                            handleCardTypeChange(card.id, value as CardType)
                          }
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="classic-flip">
                              🔄 Classic Flip
                            </SelectItem>
                            <SelectItem value="multiple-choice">
                              ✅ Multiple Choice
                            </SelectItem>
                            <SelectItem value="type-answer">
                              ⌨️ Type Answer
                            </SelectItem>
                          </SelectContent>
                        </Select>
                            ...
                            {card.frontImageUrl || card.frontImageFile ? (
                              <div className="relative inline-block">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleRemoveImage(card.id, "front")
                                  }
                              ...
                              <div>
                                <Input
                                  ...
                                  disabled={
                                    uploadingImageCardId === card.id &&
                                    uploadingImageField === "front"
                                  }
                                />
                        ...
                        {canAddImageToCard(userTier) &&
                          ...
                          card.cardType === "classic-flip" && (
                            <div className="mt-2">
                              <AudioRecorder
                                onAudioSave={(url) =>
                                  handleCardChange(card.id, { frontAudio: url })
                                }
                                currentAudioUrl={card.frontAudio}
                                onAudioRemove={() =>
                                  handleCardChange(card.id, { frontAudio: "" })
                                }
                                disabled={
                                  submitting ||
                                  (uploadingAudioCardId === card.id &&
                                    uploadingAudioField === "front")
                                }
                                label="Question Audio (Optional)"
                                onUploadAudio={(file) =>
                                  handleAudioUpload(card.id, "front", file)
                                }
                              />
                            </div>
                          )}
                          ...
                          <div>
                            ...
                            {card.incorrectAnswers.length < 5 && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleAddIncorrectAnswer(card.id)
                                }
                        ...
                        <div>
                            ...
                            {card.backImageUrl || card.backImageFile ? (
                              <div className="relative inline-block">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleRemoveImage(card.id, "back")
                                  }
                                  className="absolute top-1 right-1 h-6 w-6 p-0 bg-red-600 hover:bg-red-700 text-white rounded-full"
                                >
                                  <X className="w-3 h-3" />
                          ...
                          {canAddImageToCard(userTier) && onUploadAudio && (
                            <div className="mt-2">
                              <AudioRecorder
                                onAudioSave={(url) =>
                                  handleCardChange(card.id, { backAudio: url })
                                }
                                currentAudioUrl={card.backAudio}
                                onAudioRemove={() =>
                                  handleCardChange(card.id, { backAudio: "" })
                                }
                                disabled={
                                  submitting ||
                                  (uploadingAudioCardId === card.id &&
                                    uploadingAudioField === "back")
                                }
                                label="Answer Audio (Optional)"
                                onUploadAudio={(file) =>
                                  handleAudioUpload(card.id, "back", file)
                                }
                              />
                            </div>
                          )}
                    ...
                    disabled={
                      ...
                      cards.filter((c) => {
                        ...
                        if (c.cardType === "multiple-choice") {
                          const hasCorrect = c.correctAnswers.some((a) =>
                            a.trim()
                          );
                          const hasIncorrect = c.incorrectAnswers.some((a) =>
                            a.trim()
                          );
                          return hasFront && hasCorrect && hasIncorrect;
                        } else {
