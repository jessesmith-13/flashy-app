...
export function DeckDetailScreen() {
  ...
  const averageScore =
    studyCount > 0
      ? deckStudySessions.reduce((sum, session) => sum + session.score, 0) /
        studyCount
      : undefined;
  ...
  const [communityDeckAuthor, setCommunityDeckAuthor] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const loadCommunityDeckAuthor = async (communityDeckId: string) => {
    try {
      ...
      const communityDeck = allDecks.find(
  ...
  const handleCreateCard = async (
    ...
    try {
      ...
      const apiData: ApiCardData = {
        ...rest,
        ...(frontImageUrl !== null && frontImageUrl !== undefined
          ? { frontImageUrl }
          : {}),
        ...(backImageUrl !== null && backImageUrl !== undefined
          ? { backImageUrl }
          : {}),
        ...(frontAudio !== null && frontAudio !== undefined
          ? { frontAudio: frontAudio }
          : {}),
        ...(backAudio !== null && backAudio !== undefined
          ? { backAudio: backAudio }
          : {}),
      };
  ...
  const handleOpenEditCard = (cardId: string) => {
    ...
    if (card.cardType === "multiple-choice") {
      ...
      const incorrectAnswers =
        ...
        (card.incorrectAnswers ? card.incorrectAnswers : ["", "", ""]);
  ...
  const handleUpdateCard = async (e: React.FormEvent) => {
    ...
    if (editCardType === "multiple-choice") {
      ...
      const filledAnswers = editCardTypeAnswerAcceptedAnswers.filter((a) =>
        a.trim()
      );
    ...
    try {
      ...
      if (editCardType === "classic-flip") {
        ...
        // Filter out empty answers - multiple-choice uses correctAnswers + incorrectAnswers
        cardData.correctAnswers = editCardCorrectAnswers.filter((a) =>
          a.trim()
        );
        cardData.incorrectAnswers = editCardIncorrectAnswers.filter((a) =>
          a.trim()
        );
      } else if (editCardType === "type-answer") {
        // Filter out empty answers - type-answer requires back + acceptedAnswers
        cardData.back = editCardBack;
        cardData.acceptedAnswers = editCardTypeAnswerAcceptedAnswers.filter(
          (a) => a.trim()
        );
  ...
  const handleDeleteSelectedCards = async () => {
    ...
    try {
      ...
      console.log(
        `🗑️ Bulk delete: ${deckCards.length} cards -> ${newCardCount} remaining`
      );
      updateDeck(deck.id, { cardCount: newCardCount });
  ...
  const handleToggleFavorite = async (cardId: string) => {
    ...
    try {
      ...
      toast.success(
        card.favorite ? "Removed from favorites" : "Added to favorites"
      );
    } catch (error) {
      handleAuthError(error);
  ...
  const handleUpdateDeck = async (e: React.FormEvent) => {
    ...
    try {
      const updates = {
        ...
        difficulty: (editDifficulty || undefined) as
          | DifficultyLevel
          | undefined,
  ...
  const handlePublishDeck = async () => {
    ...
    setPublishing(true);
    try {
      await apiPublishDeck(accessToken, deck.id, {
  ...
  const handleBulkAddCards = async (cardsData: CardFormData[]) => {
    ...
    try {
      console.log(
        `🚀 Starting bulk card creation for ${cardsData.length} cards`
      );
        ...
        try {
          ...
          if (cardInput.cardType === "classic-flip") {
            ...
            cardData.correctAnswers =
              cardInput.correctAnswers?.filter((a: string) => a.trim()) || [];
            cardData.incorrectAnswers =
              cardInput.incorrectAnswers?.filter((a: string) => a.trim()) || [];
          } else if (cardInput.cardType === "type-answer") {
            // Type-answer: requires back + optional acceptedAnswers
            cardData.back = cardInput.back || "";
            cardData.acceptedAnswers =
              cardInput.acceptedAnswers?.filter((a: string) => a.trim()) || [];
          }
          ...
          if (
            cardInput.cardType === "multiple-choice" &&
            correctAnswers &&
            incorrectAnswers
          ) {
            apiData.correctAnswers = correctAnswers;
      ...
      if (cardsToCreate.length > 0) {
        ...
        if (createdCards && Array.isArray(createdCards)) {
          ...
          console.log(
            `✅ Successfully batch created ${createdCards.length} cards`
          );
        }
      }
      ...
      if (deck) {
        updateDeck(deck.id, {
          cardCount: (deck.cardCount || 0) + cardsToCreate.length,
        });
      }
      ...
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-4 sm:p-6 lg:p-8">
        <div className="max-w-6xl mx-auto">
          <DeckHeader
            ...
            canPublish={canPublishToCommunity(
              user?.subscriptionTier,
              user?.isSuperuser,
              user?.isModerator
            )}
