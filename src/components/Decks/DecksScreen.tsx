...
export function DecksScreen() {
  ...
  const handleDrop = async (targetDeckId: string) => {
    ...
    try {
      await updateDeckPositions(
        accessToken,
        updatedDecks.map((d) => ({ id: d.id, position: d.position }))
      );
    } catch (error) {
  ...
  const handlePublishToCommunity = async () => {
    ...
    try {
      ...
      const result = await publishDeck(accessToken, publishingDeck.id, {
      ...
      console.log(
        "✅ Updated deck in store with communityPublishedId:",
        result.deck?.id
      );
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(
            "🔄 Reloaded decks after publish:",
            freshDecks
              .filter((d: UIDeck) => d.communityPublishedId)
              .map((d: UIDeck) => ({
                name: d.name,
      ...
      if (error.message?.includes("No changes detected")) {
        toast.info(
          "This deck is already published with no changes. Make edits to the deck to publish an update."
        );
      } else {
  ...
  const confirmUnpublish = async () => {
    ...
    try {
      console.log(
        "📤 Calling unpublishDeck with deck ID:",
        unpublishingDeck.id
      );
  ...
  const handleUpdateDeck = async (data: {
    ...
    if (
      editingDeck.communityPublishedId &&
      !editingDeck.sourceCommunityDeckId
    ) {
      try {
        const deckCards = await fetchCards(accessToken, editingDeck.id);
        await updateCommunityDeck(
          accessToken,
          editingDeck.communityPublishedId,
          {
            ...data,
  ...
  const filteredDecks = decks.filter((deck) => {
    ...
    const result =
      tabFilter && searchFilter && categoryFilter && subtopicFilter;
  ...
  const unpublishedCount = decks.filter(
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="learned">
                    Learned ({decks.filter((d) => d.isLearned).length})
                  </SelectItem>
                  <SelectItem value="added">
                    Added (
                    {
                      decks.filter(
                        (d) =>
                          (d.sourceCommunityDeckId &&
                            !d.communityPublishedId) ||
                          d.isShared
                      ).length
                    }
                    )
            ...
            <div className="hidden sm:block">
              ...
              >
                <TabsList className="bg-white dark:bg-gray-800 shadow-sm inline-flex">
                  <TabsTrigger
                    value="all"
                    className="data-[state=active]:bg-emerald-100 dark:data-[state=active]:bg-emerald-900/30 data-[state=active]:text-emerald-700 dark:data-[state=active]:text-emerald-400 text-sm whitespace-nowrap"
                  >
                    All
                  ...
                  <TabsTrigger
                    value="added"
                    className="data-[state=active]:bg-emerald-100 dark:data-[state=active]:bg-emerald-900/30 data-[state=active]:text-emerald-700 dark:data-[active]:text-emerald-400 text-sm whitespace-nowrap"
                  >
                  ...
                  >
                    ...
                    <div className="w-full">
                      ...
                      >
                        ...
                        <h3 className="mb-2 truncate dark:text-gray-100">
                          {deck.name}
                        </h3>
                      ...
                      <div className="flex items-center gap-1 mt-3">
                        ...
                        >
                          <Star
                            className={`w-4 h-4 ${
                              deck.isFavorite ? "fill-yellow-500" : ""
                            }`}
                          />
                        </Button>
                        <Button
                        ...
                        >
                          <CheckCircle
                            className={`w-4 h-4 ${
                              deck.isLearned ? "fill-emerald-600" : ""
                            }`}
                          />
                        </Button>
                        <Button
                        ...
                        <AlertDialog key={`delete-${deck.id}`}>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                This will permanently delete "{deck.name}" and
                          ...
                          (!deck.category || !deck.subtopic) && (
                            <Button
                              ...
                              onClick={(e) => {
                                ...
                                toast.info(
                                  `Add ${
                                    !deck.category ? "a category" : "a subtopic"
                                  } to publish this deck`
                                );
                              ...
                              title={`Add ${
                                !deck.category ? "category" : "subtopic"
                              } to publish`}
                            >
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {publishingDeck.cardCount || 0}{" "}
                      {publishingDeck.cardCount === 1 ? "card" : "cards"}
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {unpublishingDeck.cardCount || 0}{" "}
                      {unpublishingDeck.cardCount === 1 ? "card" : "cards"}
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 pt-2">
