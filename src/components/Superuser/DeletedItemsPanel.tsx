...
export function DeletedItemsPanel({ accessToken }: DeletedItemsPanelProps) {
  ...
  const sortedComments = useMemo(() => {
    return [...comments].sort((a, b) => new Date(b.deletedAt).getTime() - new Date(a.deletedAt).getTime())
  }, [comments])

  const sortedDecks = useMemo(() => {
    return [...decks].sort((a, b) => new Date(b.deletedAt).getTime() - new Date(a.deletedAt).getTime())
  }, [decks])

  const sortedCards = useMemo(() => {
    return [...cards].sort((a, b) => new Date(b.deletedAt).getTime() - new Date(a.deletedAt).getTime())
  }, [cards])

  const handleRestoreClick = (id: string, type: 'comment' | 'deck' | 'card', name: string) => {
  ...
  const handleRestoreConfirm = async () => {
    ...
    try {
      ...
      toast.success(`${selectedItem.type.charAt(0).toUpperCase() + selectedItem.type.slice(1)} restored successfully`)
    ...
    <div className="space-y-6">
      ...
      {totalDeleted === 0 ? (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600">
          ...
          <h3 className="text-gray-700 dark:text-gray-300 mb-2">No Deleted Items</h3>
            ...
            <ScrollArea className="h-[500px] rounded-xl border border-gray-200 dark:border-gray-700">
              {sortedComments.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">No deleted comments</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleRestoreClick(comment.id, 'comment', 'Comment')}
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs text-gray-700 dark:text-gray-300">
                                <span className="font-medium">Reason:</span> {comment.deletedReason}
                              </p>
                            </div>
                          </div>
                        </div>
            ...
            <ScrollArea className="h-[500px] rounded-xl border border-gray-200 dark:border-gray-700">
              {sortedDecks.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">No deleted decks</p>
                </div>
              ) : (
                <div className="space-y-3 p-4">
                    ...
                    >
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleRestoreClick(deck.id, 'deck', deck.name)}
                      ...
                      {deck.deletedReason && (
                        <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs text-gray-700 dark:text-gray-300">
                                <span className="font-medium">Reason:</span> {deck.deletedReason}
                              </p>
                            </div>
                          </div>
                        </div>
            ...
            <ScrollArea className="h-[500px] rounded-xl border border-gray-200 dark:border-gray-700">
              {sortedCards.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">No deleted cards</p>
                </div>
              ) : (
                <div className="space-y-3 p-4">
                    ...
                    >
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleRestoreClick(card.id, 'card', 'Card')}
                      ...
                      {card.deletedReason && (
                        <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs text-gray-700 dark:text-gray-300">
                                <span className="font-medium">Reason:</span> {card.deletedReason}
                              </p>
                            </div>
                          </div>
                        </div>
      ...
      <AlertDialog open={restoreDialogOpen} onOpenChange={setRestoreDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to restore this {selectedItem?.type}? It will be visible to users again.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
