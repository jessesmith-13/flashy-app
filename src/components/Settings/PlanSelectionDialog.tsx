    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-[1400px] max-h-[90vh] overflow-y-auto w-[96vw] sm:!max-w-[1400px]">
        <DialogHeader className="pb-2">
          ...
          <DialogDescription>
            Choose the plan that works best for you. You can change or cancel
            anytime.
          </DialogDescription>
        </DialogHeader>
            ...
            return (
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  {plan.savings && (
                    <div
                      className={`inline-block ${plan.badgeColor} text-xs font-medium px-2.5 py-1 rounded-full`}
                    >
                      {plan.savings}
                    </div>
                  )}
                ...
                <ul className="space-y-2 mb-6 flex-1">
                  {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-start gap-2 text-sm">
                      <Check
                        className={`w-4 h-4 ${plan.checkColor} flex-shrink-0 mt-0.5`}
                      />
                ...
                <Button
                  onClick={() =>
                    onSelectPlan(plan.id as "monthly" | "annual" | "lifetime")
                  }
                  disabled={isCurrent}
                  className={`w-full ${
                    isCurrent
