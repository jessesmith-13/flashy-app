...
function isAccountBannedError(err: unknown): err is AccountBannedError {
  return err instanceof Error && err.name === "ACCOUNT_BANNED";
}
...
export function SignUpScreen() {
  ...
  // NEW: Handler for role selection continuation
  ...
  const handleSignup = async (
    ...
    if (!acceptedTerms || !acceptedPrivacy) {
      setError(
        "You must accept the Terms of Use and Privacy Policy to sign up"
      );
      return;
    }
    ...
    try {
      ...
      if (session) {
        ...
        if (returnToSharedDeck) {
          console.log(
            "SignUpScreen - Returning to shared deck after signup:",
            returnToSharedDeck
          );
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      // NEW: Store selected role for OAuth callback
      if (selectedRole) {
  ...
  const handleContinue = () => {
    ...
    if (returnToSharedDeck) {
      console.log(
        "SignUpScreen - Returning to shared deck after continue:",
        returnToSharedDeck
      );
      sessionStorage.removeItem("returnToSharedDeck");
      window.location.hash = `#/shared/${returnToSharedDeck}`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
      <div className="w-full max-w-3xl bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 sm:p-10">
        <AuthHeader
        ...
        {showSuccess ? (
          ...
          // STEP 1: Show role selector first
          <UserRoleSelector
            selectedRole={selectedRole}
            onRoleSelect={setSelectedRole}
            onContinue={handleRoleContinue}
            showVerificationCTA={false} // Don't show during signup
          />
        ) : (
          // STEP 2: Show signup form after role selection
          <>
            {referralCode && (
              <div className="mb-6 p-4 bg-gradient-to-r from-emerald-50 to-blue-50 dark:from-emerald-950/30 dark:to-blue-950/30 border border-emerald-200 dark:border-emerald-800 rounded-lg">
                <div className="flex items-center gap-2 text-emerald-900 dark:text-emerald-100">
                  ...
                  <span className="text-sm">
                    You were invited by a friend! Sign up to get{" "}
                    <strong>1 month of Premium free</strong>
                  </span>
                </div>
              </div>
            )}
