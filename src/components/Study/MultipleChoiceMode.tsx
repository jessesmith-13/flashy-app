...
export function MultipleChoiceMode({
  ...
  const handleNext = () => {
    ...
    const isCorrect =
      correctlySelected === totalCorrect && incorrectlySelected === 0;
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      await apiUpdateCard(accessToken, selectedDeckId, currentCard.id, {
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4 sm:mb-6">
            <div className="text-xs text-blue-600 dark:text-blue-400 uppercase tracking-wide text-center">
              Multiple Choice{" "}
              {hasMultipleCorrect && `(Select ${correctAnswers.length})`}
            </div>
            {currentCard.front && (
              <Button
                variant="ghost"
              ...
              >
                <Volume2
                  className={`w-4 h-4 ${
                    isSpeaking ? "text-blue-600 animate-pulse" : "text-gray-500"
                  }`}
                />
              </Button>
            )}
              ...
              >
                {isCorrect ? (
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      The correct{" "}
                      {correctAnswers.length > 1 ? "answers are" : "answer is"}:{" "}
