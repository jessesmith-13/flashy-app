...
export function TicketManagementPanel() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "open" | "reviewing" | "resolved" | "all"
  >("open");
  const [typeFilter, setTypeFilter] = useState<
    "all" | "deck" | "user" | "comment" | "card"
  >("all");
  ...
  const filteredTickets = useMemo(() => {
    ...
    if (showFlashyOnly) {
      filtered = filtered.filter(
        (ticket) => ticket.flaggedUserDisplayName?.toLowerCase() === "flashy"
      );
    }
  ...
  const handleChangeStatus = async (
    ...
    try {
      await api.updateTicketStatus(accessToken, ticketId, {
        ...
        resolutionNote:
          newStatus === "resolved" ? "Resolved from ticket list" : undefined,
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Button
            ...
            className={
              showEscalatedOnly ? "bg-red-600 hover:bg-red-700 text-white" : ""
            }
          >
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        Content author:{" "}
                        {ticket.flaggedUserDisplayName || "Unknown"}
                      </p>
                    </div>
                      ...
                      <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 space-y-1.5 border border-red-200 dark:border-red-800">
                        ...
                        {ticket.escalatedAt && (
                          <div className="text-xs text-gray-700 dark:text-gray-300">
                            ...
                            {new Date(ticket.escalatedAt).toLocaleTimeString(
                              [],
                        ...
                        {ticket.resolutionReason && (
                          <div className="text-xs text-gray-700 dark:text-gray-300">
                            ...
                            <span className="capitalize">
                              {ticket.resolutionReason === "rejected" &&
                                "❌ Rejected - Not a valid report"}
                              {ticket.resolutionReason === "approved" &&
                                "✅ Approved - Flag is valid"}
                              {ticket.resolutionReason === "removed" &&
                                "🗑️ Removed - Content deleted"}
                            </span>
                          </div>
                        )}
                        {ticket.resolvedBy && (
              ...
              selectedTicket
                ? {
                    ...
                    targetName:
                      ...
                      "Unknown",
                    reason:
                      selectedTicket.flagReason || selectedTicket.category,
                    reporterNotes:
                      selectedTicket.flagAdditionalDetails ||
                      selectedTicket.description ||
                      "",
                  }
                : undefined
