  ...
  return (
    <div className={cardClassName}>
      ...
      >
        ...
        <h3 className="text-base sm:text-lg text-gray-900 dark:text-gray-100 mb-1 break-words">
          {deck.name}
        </h3>
        ...
        <div className="text-xs text-gray-500 dark:text-gray-500 space-y-0.5">
          ...
          {deck.publishedAt && (
            <div>
              Updated:{" "}
              {new Date(deck.updatedAt || deck.publishedAt).toLocaleDateString(
                "en-US",
                {
                  year: "numeric",
                  month: "short",
      ...
      {isSuperuser && (
        <div className="flex gap-2 mb-3">
          ...
          >
            <Star
              className={`w-3 h-3 mr-1 ${deck.featured ? "fill-current" : ""}`}
            />
      ...
      {buttonState === "update-available" && importedDeck ? (
        ...
        <Button
          ...
          disabled={addingDeckId === deck.id}
          className={`w-full bg-emerald-600 hover:bg-emerald-700 text-white ${
            isFeatured ? "text-sm sm:text-base h-9 sm:h-10" : ""
          }`}
        >
          <Plus className="w-4 h-4 mr-2" />
          {addingDeckId === deck.id ? "Adding..." : "Add to My Decks"}
