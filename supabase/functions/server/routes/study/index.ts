...
function checkAchievements(stats: any): AchievementUpdate[] {
  ...
  if (stats.perfect_scores >= 1) {
    unlock(
      ...
      "💯",
      "Perfect Score",
      "Get every card correct in a study session",
    );
  }
  ...
  if (stats.cards_reviewed >= 100 && (stats.average_accuracy || 0) >= 90) {
    unlock(
      ...
      "🎖️",
      "Accuracy Expert",
      "Maintain 90%+ accuracy over 100 cards",
    );
  }
  ...
  if (stats.studied_sixty_minutes_nonstop) {
    unlock(
      ...
      "🏃",
      "Marathon Session",
      "Study for 60+ minutes without stopping",
    );
  }
  ...
  if (stats.completed_beginner_deck) {
    unlock(
      ...
      "🟢",
      "Beginner Master",
      "Complete a beginner difficulty deck",
    );
  }
  if (stats.completed_intermediate_deck) {
    unlock(
      ...
      "🟡",
      "Intermediate Master",
      "Complete an intermediate difficulty deck",
    );
  }
  if (stats.completed_advanced_deck) {
    unlock(
      ...
      "🟠",
      "Advanced Master",
      "Complete an advanced difficulty deck",
    );
  }
  if (stats.completed_expert_deck) {
    unlock(
      ...
      "🔴",
      "Expert Master",
      "Complete an expert difficulty deck",
    );
  }
  if (stats.completed_master_deck) {
    unlock(
      ...
      "🌈",
      "Ultimate Master",
      "Complete a master difficulty deck",
    );
  }
  ...
  if (stats.slow_card_review) {
    unlock(
      ...
      "🐢",
      "Slow and Steady",
      "Take over 10 minutes to review one card",
    );
  }
  if (stats.studied_on_low_battery) {
    unlock(
      ...
      "🔋",
      "Dedication Mode",
      "Study while your battery is below 5%",
    );
  }
...
async function checkStudiedThreeHoursToday(
  userId: string,
...
async function updateAchievementStats(
  ...
  const cardsReviewed =
    (currentStats.cards_reviewed || 0) + sessionData.cardsStudied;
  const totalStudyMinutes = Math.floor(
    ((currentStats.total_study_minutes || 0) * 60 +
  ...
  const totalPreviousCorrect = Math.round(
    (previousCorrect * previousAccuracy) / 100,
  );
  const totalCorrect = totalPreviousCorrect + sessionData.correctCount;
  const averageAccuracy =
    cardsReviewed > 0 ? Math.round((totalCorrect / cardsReviewed) * 100) : 0;
  ...
  const perfectScores =
    (currentStats.perfect_scores || 0) + (isPerfectScore ? 1 : 0);
  ...
  const studiedAfterMidnight =
    currentStats.studied_after_midnight || (hour >= 0 && hour < 3);
  const studiedSixtyMinutesNonstop =
    currentStats.studied_sixty_minutes_nonstop ||
    sessionData.timeSpentSeconds >= 3600;
  ...
  const studiedOnLowBattery =
    currentStats.studied_on_low_battery || sessionData.lowBattery === true;

  // Slow card review check
  const avgTimePerCard =
    sessionData.cardsStudied > 0
      ? sessionData.timeSpentSeconds / sessionData.cardsStudied
      : 0;
  const slowCardReview = currentStats.slow_card_review || avgTimePerCard > 600; // 10 minutes per card

  // Difficulty completion tracking
  const completedBeginnerDeck =
    currentStats.completed_beginner_deck ||
    sessionData.deckDifficulty === "beginner";
  const completedIntermediateDeck =
    currentStats.completed_intermediate_deck ||
    sessionData.deckDifficulty === "intermediate";
  const completedAdvancedDeck =
    currentStats.completed_advanced_deck ||
    sessionData.deckDifficulty === "advanced";
  const completedExpertDeck =
    currentStats.completed_expert_deck ||
    sessionData.deckDifficulty === "expert";
  const completedMasterDeck =
    currentStats.completed_master_deck ||
    sessionData.deckDifficulty === "master";
  ...
  if (newAchievements.length > 0) {
    ...
    const allUnlockedIds = [
      ...(updatedStats.unlocked_achievement_ids || []),
      ...newUnlockedIds,
    ];
    ...
    console.log(
      `🎉 User ${userId} unlocked ${newAchievements.length} new achievements:`,
      newUnlockedIds,
    );
  }
...
export function registerStudyRoutes(app: Hono) {
  ...
  app.post("/study/sessions", async (c: Context) => {
    try {
      ...
      const safeSessionData = {
        cardsStudied:
          sessionData.cardsStudied ?? sessionData.cards_studied ?? 0,

        correctCount:
          sessionData.correctCount ??
          sessionData.correct_count ??
          sessionData.correctAnswers ?? // ✅ accept legacy
          0,

        incorrectCount:
          sessionData.incorrectCount ??
          sessionData.incorrect_count ??
          sessionData.incorrectAnswers ?? // ✅ accept legacy
          0,

        skippedCount:
          sessionData.skippedCount ?? sessionData.skipped_count ?? 0,
        ...
        timeSpentSeconds:
          ...
          sessionData.timeSpent ?? // ✅ accept legacy
          0,

        score: sessionData.score ?? 0,
        lowBattery: sessionData.lowBattery ?? false,
      };
      ...
      try {
        const { stats, newAchievements } = await updateAchievementStats(
          user.id,
          {
        ...
        console.error(
          "Error updating achievements (non-critical):",
          achievementError,
        );
  ...
  app.get("/study/sessions", async (c: Context) => {
    try {
      ...
      if (authError || !user) {
        console.log(
          `❌ Auth error in get study sessions: ${authError?.message}`,
        );
        return c.json({ error: "Unauthorized" }, 401);
      }
      ...
      if (dbError) {
        console.log(
          `❌ Database error fetching study sessions: ${dbError.message}`,
        );
