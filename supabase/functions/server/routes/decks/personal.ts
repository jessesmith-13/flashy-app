...
export function registerPersonalDeckRoutes(app: Hono) {
  ...
  app.post('/decks', async (c: Context) => {
    try {
      ...
      const { data: currentAchievements } = await supabase
        ...
        .select('unlocked_achievement_ids, decks_created, customized_deck_theme')
        .eq('user_id', user.id)
        .single()
      ...
      const isCustomized = (emoji && emoji !== '📚') || (color && color !== '#10B981')
      ...
      if (newDecksCreated >= 10 && !currentUnlocked.includes('deck-architect')) {
      ...
      if (categoriesUsedCount >= 5 && !currentUnlocked.includes('categorized')) {
      ...
      const { data: upsertData, error: upsertError } = await supabase
        ...
        .upsert({
          ...
          customized_deck_theme: isCustomized || currentAchievements?.customized_deck_theme || false,
      ...
      console.log(`📊 Stats - Decks created: ${newDecksCreated}, Categories used: ${categoriesUsedCount}`)
  ...
  app.put('/decks/:deckId', async (c: Context) => {
    try {
      ...
      const isCustomized = updates.emoji !== '📚' || updates.color !== '#10B981'
      ...
      if (isCustomized) {
        ...
        if (!currentUnlocked.includes('personalized')) {
          ...
          console.log(`🎉 User ${user.id} unlocked "Personalized" achievement!`)
        }
      }
  ...
  app.delete('/decks/:deckId', async (c: Context) => {
    try {
      ...
      if (authError || !user) {
        console.log(`❌ Auth error in delete personal deck: ${authError?.message}`)
        return c.json({ error: 'Unauthorized' }, 401)
      }
      ...
      if (cardsDeleteError) {
        console.log(`❌ Failed to soft delete cards: ${cardsDeleteError.message}`)
      ...
      console.log(`✅ Soft deleted personal deck ${deckId} (${deck.name}) and its cards`)
