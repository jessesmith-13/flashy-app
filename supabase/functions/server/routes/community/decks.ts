...
import { CommunityDeck, CommunityCardInsert } from '../../types/community.ts'
  ...
  app.get('/community/decks', async (c: Context) => {
    try {
      ...
      const enriched = await Promise.all(
        (decks || []).map(async (deck: Record<string, CommunityDeck[]>) => {
          ...
          return {
            ...
            authorId: deck.owner_id,   
            author: deck.owner_display_name,
