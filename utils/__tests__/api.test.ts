...
const createJsonResponse = (body: unknown, init?: { status?: number }): Response => {
...
const createTextResponse = (text: string, init?: { status?: number }): Response => {
  const responseInit: ResponseInit = {
    status: init?.status ?? 200,
    headers: {},
  }
...
describe('Auth API', () => {
  ...
  describe('resetPassword', () => {
    ...
    it('throws when resetPasswordForEmail returns an error', async () => {
      ...
      await expect(
        api.resetPassword('bad@user.com'),
...
describe('Deck API', () => {
  ...
  describe('createDeck', () => {
    ...
    it('throws when create fails', async () => {
      ...
      await expect(
        api.createDeck(accessToken, { name: '' }),
  ...
  describe('updateDeck', () => {
    it('updates a deck', async () => {
      ...
      const result = await api.updateDeck(accessToken, 'abc', { name: 'Updated' })
      expect(result).toEqual(mockDeck)
    })
  })
  ...
  describe('updateCardPositions', () => {
    it('updates card positions', async () => {
      ...
      const result = await api.updateCardPositions(accessToken, 'deck1', positions)
      expect(result).toEqual({ success: true })
    })
  })
...
describe('Card API', () => {
  ...
  describe('fetchCards', () => {
    ...
    it('throws when not ok', async () => {
      ...
      await expect(
        api.fetchCards(accessToken, deckId),
      ).rejects.toThrow('Server error')
    })
  })
...
describe('Community API', () => {
  ...
  describe('toggleCommunityDeckFeatured', () => {
    it('toggles featured status', async () => {
      ...
      const result = await api.toggleCommunityDeckFeatured(accessToken, 'community1')
      expect(result).toEqual(responseBody)
    })
  })
...
describe('Comments API', () => {
  ...
  describe('postDeckComment', () => {
    it('posts a comment and returns it', async () => {
      ...
      const result = await api.postDeckComment(accessToken, 'deck1', 'Nice deck')
...
describe('Share API', () => {
  ...
  describe('createShareLink', () => {
    it('creates a share link', async () => {
      const responseBody = { shareId: 'share123', shareUrl: '/shared/share123' }
      ;(global.fetch as unknown as vi.Mock).mockResolvedValueOnce(
        createJsonResponse(responseBody),
      )
